      ...
      <div className='flex items-center -space-x-2'>
        {sortedUsers.slice(0, 5).map((user) => (
          <div key={user.clientId} className='relative group' title={user.name}>
            <div
              className='w-8 h-8 rounded-full flex items-center justify-center text-white font-medium text-sm border-2 border-[#2e2e2e] cursor-pointer hover:scale-110 transition-transform'
            ...
            {/* Tooltip */}
            <div className='absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 bg-[#1e1e1e] text-white text-xs rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none border border-[#3e3e3e]'>
              {user.name}
            ...
            {user.clientId === localClientId && (
              <div className='absolute -bottom-1 -right-1 w-3 h-3 bg-green-500 rounded-full border-2 border-[#2e2e2e]'></div>
            )}
          </div>
        ))}
        ...
        {sortedUsers.length > 5 && (
          <div
            className='w-8 h-8 rounded-full flex items-center justify-center bg-[#3e3e3e] text-white text-xs font-medium border-2 border-[#2e2e2e]'
