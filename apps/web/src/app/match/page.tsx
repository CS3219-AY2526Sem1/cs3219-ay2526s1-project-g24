            ...
            {tabs.map((tab) => (
              <Link
                ...
                className={`font-montserrat font-medium text-sm transition-colors ${
                  activeTab === tab.name ? 'text-white' : 'text-[#9e9e9e] hover:text-white'
                }`}
      ...
      <main className='relative z-10 pt-44 pb-20 px-6 md:px-12'>
        <div className='max-w-5xl mx-auto'>
          <div className='text-center mb-16'>
            <h2 className='font-montserrat text-5xl font-semibold text-white mb-4'>Start Your Practice Session</h2>
          ...
          <section className='mb-16'>
            <h3 className='font-montserrat text-3xl font-semibold text-white mb-8'>Choose Topic</h3>
            {isLoadingTopics ? (
                  ...
                  >
                    <h4
                      className={`font-montserrat text-lg font-semibold mb-2 leading-tight ${
                        selectedTopics.includes(topic.name) ? 'text-white' : 'text-gray-400'
                      }`}
                    >
                      {topic.name}
                    </h4>
                    {topic.description && <p className='font-montserrat text-xs text-gray-500'>{topic.description}</p>}
                  </button>
                ))}
              </div>
            )}
          ...
          <section className='mb-16'>
            <h3 className='font-montserrat text-3xl font-semibold text-white mb-8'>Choose Difficulty</h3>
            <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
              {difficulties.map((diff) => (
                ...
                >
                  <div className='mb-3'>
                    <span className={`font-montserrat text-xs px-3 py-1 rounded-full ${getDifficultyStyles(diff.tag)}`}>
                      {diff.tag}
                    </span>
                  </div>
                  <h4
                    className={`font-montserrat text-2xl font-semibold mb-3 leading-tight whitespace-pre-line ${
                      selectedDifficulty === diff.level ? 'text-white' : 'text-gray-400'
                    }`}
                  >
                    {diff.level}
                  </h4>
                  <p className='font-montserrat text-sm text-gray-500'>{diff.description}</p>
                </button>
              ))}
            </div>
          </section>
          ...
          <section className='mb-12'>
            <h3 className='font-montserrat text-3xl font-semibold text-white mb-8'>Choose Coding Language</h3>
                ...
                >
                  ...
                  <h4
                    className={`font-montserrat text-xl font-medium ${
                      selectedLanguage === lang.name ? 'text-white' : 'text-gray-400'
                    }`}
                  >
                    {lang.name}
                  </h4>
