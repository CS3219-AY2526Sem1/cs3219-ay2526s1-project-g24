...
router.post("/api/v1/match/requests", async (req: Request, res: Response) => {
    ...
    try {
        await withSpan("create_match_request", { reqId }, async (span) => {
            ...
            // Extract JWT token from Authorization header or cookies (if provided)
            // This will be used to authenticate with collab service when creating session
            ...
            logger.info(
                {
                    ...
                    cookiePrefix: req.headers.cookie?.substring(0, 50),
                },
            ...
            await redisOps.createRequest(
                reqId,
                {
                    ...
                    topics: data.topics.join(","),
                    languages: data.languages.join(","),
                    ...(authToken && { authToken }), // Store token if available
                },
                0, // 0 = no expiration
            );
